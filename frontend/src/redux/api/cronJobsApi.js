...
export const cronJobsApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // Run driver-doc expiry NOW
    runDriverDocsNow: builder.mutation({
      query: ({ companyId, sendEmails = true }) => ({
        url: `/cronjobs/driver-docs/run-now`,
        method: "POST",
        body: { companyId, sendEmails },
      }),
    }),
