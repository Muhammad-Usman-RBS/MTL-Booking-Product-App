...
import { useVerifyUserOtpMutation, useResendUserOtpMutation } from "../../../redux/api/adminApi";
...
const VerificationUser = () => {
    ...
    const [verifyUserOtp, { isLoading: isVerifying }] = useVerifyUserOtpMutation();
    ...
    const canResend = useMemo(() => timer === 0 && !isResending, [timer, isResending]);
    const canVerify = useMemo(() => otp.length === 6 && !isVerifying, [otp, isVerifying]);
            ...
            <div className="min-h-[60vh] flex items-center justify-center px-4">
                <div className="max-w-md mx-auto w-full">
                    <p className="text-sm text-gray-600 mb-4">
                        We’ve sent a 6-digit code to <b>{email}</b>. Enter it within 2 minutes to create the account.
                    </p>
                    ...
                    <button
                        className={`btn btn-success w-full mb-3 ${!canVerify ? "opacity-60 cursor-not-allowed" : ""}`}
                        onClick={onVerify}
                        disabled={!canVerify}
                    >
                    ...
                    <button
                        className={`btn btn-primary w-full ${!canResend ? "opacity-60 cursor-not-allowed" : ""}`}
                        disabled={!canResend}
                        onClick={onResend}
                    >
                        {isResending ? "Sending..." : timer > 0 ? `Resend OTP in ${mm}:${ss}` : "Resend OTP"}
                    </button>
