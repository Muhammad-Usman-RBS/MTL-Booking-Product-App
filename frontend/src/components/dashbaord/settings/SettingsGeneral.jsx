...
import { colorFields, STATIC_THEME_DATA } from "../../../constants/dashboardTabsData/data";
import {
...
// Call this on user selection (only for null-company users)
const useStaticThemePicker = () => {
  const user = useSelector((s) => s.auth.user);
  const companyId = user?.companyId;

  const handlePickStaticTheme = (themeClass) => {
    if (companyId) return; // guard: DB users handled by backend
    if (!STATIC_THEMES.includes(themeClass)) return;

    // 1) apply immediately
    applyThemeClass(themeClass);

    // 2) persist
    localStorage.setItem(STATIC_THEME_KEY, themeClass);
  };

  return { handlePickStaticTheme, isStaticMode: !companyId };
};
...
const SettingsGeneral = () => {
  ...
  const { handlePickStaticTheme, isStaticMode } = useStaticThemePicker();
  ...
  const isStaticId = (id) => STATIC_THEMES.includes(id);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const seed = (STATIC_THEME_DATA || []).map((t) => ({
      _id: t.id,              // e.g. "theme-dark-1"
      themeSettings: t.colors,
      label: t.name,
    }));

    // write once to localStorage
    ...
    <div>
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    disabled={limitReached || isStaticMode}
                  />
      ...
      <div className="mt-8">
        ...
        {user?.companyId ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {STATIC_THEME_DATA.map((t) => {
              const c = t.colors || {};
              return (
                <div
                  key={t.id}
                  className="p-4 rounded-lg border transition cursor-pointer border-gray-200 bg-white hover:bg-gray-50"
                  onClick={() => handlePickStaticTheme(t.id)}
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      type="button"
                      title={
                        bookmarks.find((b) => b._id === t.id)
                          ? "Unpin from Navbar"
                          : "Pin to Navbar"
                      }
                      onClick={(e) => {
                        e.stopPropagation();
                        const isPinned = !!bookmarks.find((b) => b._id === t.id);
                        if (!isPinned && bookmarks.length >= 3) {
                          toast.error("You can pin up to 3 themes in the navbar.");
                          return;
                        }
                        // store full colors so navbar previews look right
                        dispatch(
                          toggleBookmarkTheme({
                            _id: t.id,
                            themeSettings: t.colors,
                        ...
                        // persistence handled by the useEffect in 1A
                      }}
                    >
                      {bookmarks.find((b) => b._id === t.id) ? (
                        <Icons.BookmarkCheck className="w-5 h-5 text-blue-600" />
