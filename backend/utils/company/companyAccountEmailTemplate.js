export default function companyAccountEmailTemplate({
  company = {},
  options = {},
}) {
  const brand = {
  ...
  const logoUrl = absUrl(company.profileImage) || absUrl(options.logoUrl) || "";
  const fields = [
  ...
  const card = ({ label, value, isLink, isEmail }) => {
    const val =
      value === null || value === undefined || value === ""
        ...
        ? `<a href="${safe(
            ...
            brand.accent
          };text-decoration:none">${safe(String(value))}</a>`
        : isEmail
    ...
    return `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
             style="background:${brand.card};border:1px solid ${
      brand.border
    };border-radius:12px">
        <tr>
          <td style="padding:12px 14px">
            <div style="font:700 11px/1 Arial, sans-serif;color:${
              brand.muted
...
<body style="margin:0;padding:0;background:${brand.bg}">
    ...
    <tr>
      <td align="center" style="padding:28px 16px">
        <table role="presentation" width="680" cellpadding="0" cellspacing="0" border="0"
               class="container"
               style="width:680px;max-width:680px;background:${
                 brand.card
               };border:1px solid ${
    brand.border
                  ...
                  <td class="logo" align="left" style="vertical-align:middle">
                    ${
                      logoUrl
                        ? `<img src="${safe(
                            logoUrl
                          )}" width="96" style="display:block;height:auto;border-radius:12px;border:1px solid ${
                            brand.border
                          }" alt="${safe(company.companyName || "Company")}">`
                        : `<div style="font:800 18px/1 Arial,sans-serif;color:${
                            brand.primary
                          }">${safe(company.companyName || "Company")}</div>`
                    }
                  </td>
          ...
          <tr>
            <td class="px" style="padding:22px 22px 10px">
              <div style="font:700 22px/1.35 Arial, sans-serif;color:${
                brand.primary
              };margin:0 0 6px">
                Company Account Details
              </div>
              <div style="font:400 14px/1.65 Arial, sans-serif;color:${
                brand.muted
              }">
  ...
  <td style="padding:20px 24px;border-top:1px solid ${
    ...
    <div style="font:500 12px/1.4 Arial,sans-serif;color:${
      brand.muted
    };margin-bottom:8px">
