...
//       from: `"MTL Booking" <${process.env.BREVO_SENDER_EMAIL}>`,
//       to,
//       subject,
//       html: finalHtml,
//       text: textAlt,
...
// Function to render a text fallback (for email clients that don't support HTML)
const renderTextFallback = (subject, title, subtitle, data = {}) => {
  const lines = [];
  if (title) lines.push(title);
  if (subtitle) lines.push(subtitle);
...
const sendEmail = async (to, subject, payload = {}) => {
  ...
  const transporter = nodemailer.createTransport({
    host: 'smtp-relay.brevo.com',
    port: 465, // SSL port for Brevo
    secure: true, // Use SSL
    auth: { 
      user: process.env.BREVO_SENDER_EMAIL, // Email from environment variable
      pass: process.env.BREVO_API_KEY,      // API key from environment variable
    },
    connectionTimeout: 30000,  // 30 seconds timeout for connection
    greetingTimeout: 30000,    // 30 seconds timeout for greeting
  ...
  try {
    // Send email
    const info = await transporter.sendMail({
      from: `"MTL Booking" <${process.env.BREVO_SENDER_EMAIL}>`, // Sender email
      to,  // Receiver email
      subject,  // Subject line
      html: finalHtml,  // HTML content
      text: textAlt,    // Plain text fallback
    });
    console.log(`Email sent to: ${to}, MessageId: ${info.messageId}`);
  } catch (error) {
    console.error("Error sending email:", error.message);
    console.error("Full error:", error);  // Full error log
