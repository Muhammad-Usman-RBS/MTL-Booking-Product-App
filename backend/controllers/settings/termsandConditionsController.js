...
export const createTerms = async (req, res) => {
  try {
    ...
    if (req.user.role === "superadmin") {
      // Superadmin can only target clientadmin
      ...
      // ✅ always include clientadmin themselves
  ...
  try {
    ...
    if (userRole === "superadmin") {
      ...
      query = {
        $or: [
          {
            // Terms created by clientadmin for their company
...
export const updateTerms = async (req, res) => {
  try {
    ...
    const isCreator = existingTerm.createdBy.toString() === req.user._id.toString();
    ...
    existingTerm.targetAudience = targetAudience; // ✅ update this field
...
export const deleteTerms = async (req, res) => {
  ...
  try {
    ...
    console.log("Error in deleteTerms controller", error);
    return res
      .status(500)
