...
const validateThemeSettings = (themeSettings) => {
  ...
  for (const key of requiredKeys) {
    ...
    if (!isValidHexColor(themeSettings[key])) {
      return { isValid: false, message: `Invalid hex color format for ${key}: ${themeSettings[key]}` };
    }
  }
...
export const getAllThemes = async (req, res) => {
  try {
    const themes = await Theme.find().select('companyId themeSettings createdAt updatedAt');
